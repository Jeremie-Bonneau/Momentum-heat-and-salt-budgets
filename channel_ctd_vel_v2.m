...
%% ====================   Momentum Budget ========================
x=[0:4200];
A=interp1([0 4200],[16.6 10],x);
d=interp1([0 4200],[7.5 2.5],x);
B=interp1([0 4200],[20 75],x);

rho_o=c_o.rho;
rho_o(1:24)=1005; %has no influence on the results

rho_c=c_c.rho;
rho_c(1:26)=1000; %just for interolation

rho_b=c_b.rho;
rho_b(1:71)=1000;  %just for interolation

gpB=9.8*(rho_o-rho_b)./1020; %g' at StB
gpC=9.8*(rho_o-rho_c)./1020; %g' at CtC

for i=1:180
    gp(i,:)=interp1([0 4200],[gpB(i) gpC(i)],x); %interplation of g' along x
end

% buoyancy term
dddx=-5/4200;
for i=1:4201
    idxt(i)=round(d(i)*10); %apex of channel
    idxb(i)=round(A(i)*10); %bottom of plume
    gpm(i)=mean(gp(idxt(i):idxb(i),i));
    gpm(i)=gpm(i).*(A(i)-d(i)); %mean g' over plume
    buoyancyI(i)=sum(gpm(i).*B(i).*dddx); %function to integrate along x : g'*D*B*dd_dx
end

buoyancy=sum(buoyancyI) %integral value
...
inertia=Uct-Ubt %inertia term, can be calculated as the difference btwn the 2 xsections

% drag term
drag=inertia+buoyancy

D=A-d; %plume thickness
U=interp1([0 4200],[Ub Uc],x); 
Cd=-drag/(sum((B+D+D).*U.^2)) %drag coefficient value




%% eta for a 10cm difference btw offshore and fjord
deta=0.1*4.2/20.2;
eta=interp1([0 4200],[-deta 0],x);
e=B.*(A-d).*9.8.*(eta(end)-eta(1))./4200;
eta=sum(e)
